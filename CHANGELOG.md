...
- **UI / Badge d'estat**: es recupera el tractament visual correcte dels estats després de la unificació, amb color semàntic visible i capitalització natural (`Nou`, `En curs`, `Pausada`...) en lloc de text en majúscules totals.
- **Llistats paginats / Hover d'accions**: els botons `Obrir`, `Convertir`, `Arxivar`, `Activar` i `Duplicar` adopten ara hovers semàntics amb text i icona del mateix color fort i un fons tintat més subtil, evitant l'efecte de requadre neutre desenganxat dins de la càpsula d'accions.
